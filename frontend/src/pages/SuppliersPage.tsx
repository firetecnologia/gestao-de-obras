...
export default function SuppliersPage() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true)
    try { const res = await suppliersApi.list({ search, page_size: 50 }); setSuppliers(res.data.items) } catch { /* empty */ } finally { setLoading(false) }
  }, [search])
  ...
  const handleDelete = async (id: string) => {
    if (confirm("Excluir este fornecedor?")) { await suppliersApi.delete(id); fetchData() }
  }
